...
p1_cls <- df_comp %>%
  ...
  filter(depth !=10  & depth !=20 & depth !=800) %>%
  filter(key == "Purity") %>%
...
# add snreads from df_merge
df_merge_cls <- df_merge %>%
  filter(! is.na(cls)) %>%
  select(benchmark, profile, id, depth, consensus, cls, snreads) %>%
  distinct() %>%
  mutate(qseq = paste0("si", id, depth, "_", cls)) %>% #nolint
  # snreads >= 20
  filter(snreads >= 20) %>%
  group_by(benchmark, profile, id, depth, consensus) %>% #nolint
  mutate(ncls = length(unique(cls))) %>% #nolint
  ungroup() %>%
  select(benchmark, profile, consensus, snreads, qseq, ncls)

df <- df %>%
  right_join(df_merge_cls, by = c("benchmark", "profile", "consensus", "qseq")) #nolint
```
...
p3 <- df123 %>%
  ...
  # add line y = 0, 10 refs in two strands
...
summ_err <- df %>%
  ...
  filter(asm == "asm10") %>%
  group_by(benchmark, profile, id, depth, asm, consensus) %>%
  summarise(alnerr = mean(100 - alnid),
            qcov_un = mean(100 - qcov),
            tcov_un = mean(100 - tcov),
            total_err = alnerr + qcov_un,
            n = n(),
            ) %>%
  # sort by benchmark, profile, id, total_err in descending order
  arrange(benchmark, depth, desc(total_err))
write.csv(summ_err, "./table/simulate/summ_err.csv", row.names = FALSE, quote = FALSE) #nolint 
```
