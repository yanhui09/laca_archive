...
fs_all <- list.files(
  path = "./data/simulate/umapclust_mauto",
  pattern = "*.paf", full.names = TRUE, recursive = TRUE)
fs <- grep(pattern = '.*/cut/.*.paf', fs_all, value = TRUE)

dt <- rbindlist(sapply(fs, function(x) fread(x, header=FALSE), simplify = FALSE), idcol = "file", use.names = FALSE) #nolint
dt$benchmark <- gsub("^.*simulate/umapclust_mauto/(nanopore2023duplex|pacbio2016ccs)/(even10|skew10)/.*", "\\1", dt$file) #nolint
dt$profile <- gsub("^.*simulate/umapclust_mauto/(nanopore2023duplex|pacbio2016ccs)/(even10|skew10)/.*", "\\2", dt$file) #nolint
dt$id <- as.numeric(
  gsub("^.*/si(9095|9597|9798|9899|99100).*", "\\1", dt$file)) #nolint
dt$depth <- as.numeric(
  gsub("^.*/si(9095|9597|9798|9899|99100)([0-9]+).*", #nolint
...
sum_table22 <- rbind.data.frame(sum_table2, sum_table21)

dir.create("table/simulate", showWarnings = FALSE, recursive = TRUE)
write.csv(sum_table1, "table/simulate/sum_table1.csv", row.names = FALSE, quote = FALSE) #nolint
write.csv(sum_table22, "table/simulate/sum_table2.csv", row.names = FALSE, quote = FALSE) #nolint
```
