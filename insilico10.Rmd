...
p1_cls <- df_comp %>%
  ...
  geom_col(position = position_dodge2(width = 0.8, preserve = "single")) + #nolin
  labs(
...
df_merge <- df_cls %>%
  ...
  ungroup() %>%
  # number of reads supporting the class
  group_by(benchmark, profile, id, depth, consensus, cls, ref) %>%
  mutate(snreads2 = length(unique(seqid))) %>%
  # sort by nsreads, the most common class comes first
  arrange(desc(snreads2), ref, .by_group = TRUE) %>%
  ungroup()
...
df_comp0 <- df_merge %>%
  ...
  summarize(
    Purity = purity(as.integer(as.ordered(cls)), as.integer(as.ordered(ref))), #nolint
  ) %>%
  ungroup() %>%
...
df_comp <- df_merge %>%
  ...
  summarise(
    Size = log2(length(unique(cls))),
    Purity = purity(as.integer(as.ordered(cls)), as.integer(as.ordered(ref))),
    NMI = NMI(as.integer(as.ordered(ref)), as.integer(as.ordered(cls))),
  ) %>%
...
p1_cls_size_nmi <- df_comp %>%
  ...
  geom_hline(data = data.frame(yint=c(log2(10),1),key=c("Size", "NMI")), aes(yintercept = yint), linetype = "dashed") + #nolint
  theme_prism() +
  theme(
    legend.title = element_text(),
    axis.text.x.bottom = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 9) #nolint
...
p4_alnid <- df %>%
  ...
  filter(depth != 10 & depth != 800) %>%
  filter(asm == "asm10") %>%
  ggplot(aes(x = as.factor(depth), y = alnid, color = mapq)) +
...
p4_qcov <- df %>%
  ...
  filter(depth != 10 & depth != 800) %>%
  filter(asm == "asm10") %>%
  ggplot(aes(x = as.factor(depth), y = qcov, color = mapq)) +
  geom_quasirandom(aes(shape = profile)) +
  labs(
    x = "",
    y = "% of matches in query sequence",
    color = "MAPQ",
    shape = "Simulation profile"
   ) +
  scale_color_distiller(palette = "Spectral") +
...
p4_tcov <- df %>%
  ...
  filter(depth != 10 & depth != 800) %>%
  filter(asm == "asm10") %>%
  ggplot(aes(x = as.factor(depth), y = tcov, color = mapq)) +
  geom_quasirandom(aes(shape = profile)) +
  labs(
    x = "",
    y = "% of matches in target sequence",
    color = "MAPQ",
    shape = "Simulation profile"
   ) +
  scale_color_distiller(palette = "Spectral") +
...
p5 <- df %>%
  mutate(
    ...
    consensus = factor(
      ...
      levels = c("kmerCon", "miniCon", "isoCon"))) %>%
  filter(depth != 10 & depth != 800) %>%
  filter(asm == "asm10", id == "1~2%") %>%
  select(qseq, mapq, benchmark, profile, depth, alnid, qcov, tcov, consensus) %>% #nolint 
  gather(key = "stats", value = "value", -qseq, -mapq, -benchmark, -profile, -depth, -consensus) %>% #nolint 
  ggplot(aes(x = as.factor(depth), y = value, color = mapq)) +
  geom_quasirandom(aes(shape = profile)) +
  labs(
    x = "",
  ...
  scale_color_distiller(palette = "Spectral") +
  facet_grid(stats ~ benchmark + consensus) +
  theme_prism() +
  theme(
    legend.title = element_text(),
    axis.text.x.bottom = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
ps <- list(p2, p3)
pnames <- c("p2_refcov", "p3_tseqsnum")
ps1 <- list(p4_alnid, p4_qcov, p4_tcov)
pnames1 <- c("p4_alnid", "p4_qcov", "p4_tcov")
ggsave2 <- function(x, y, width = 18, height = 12) {
  ggsave(
...
ggsave2("p1_cls", p1_cls, width = 10, height = 4)
ggsave2("p1", p1, width = 12, height = 10)
mapply(ggsave2, pnames, ps, 10, 10)
mapply(ggsave2, pnames1, ps1)
ps2 <- list(p1_pct_clust,p1_clustid, p0_cls)
pnames2 <- c("p1_pct_clust", "p1_clustid", "p0_cls")
mapply(ggsave2, pnames2, ps2, 16, 10)
ggsave2("p5_stats9899", p5, 20, 6)
```
