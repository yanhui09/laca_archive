...
countab <- read.table("data/emu/emu_demultiplexed_b0_estimated.tsv", header = T, sep = "\t", row.names = 1, comment.char = "") # nolint
...
merge_otus_sum <- function(ps_list, n = 0, taxal = "Species", binary = FALSE,
 con_cv, bin_cv, rarefy_size = NULL) {
  otus_sum_list <- lapply(seq_along(ps_list), function(i) {
    ...
    con_c = con_cv[i], bin_c = bin_cv[i], rarefy_size = rarefy_size)
  })
...
otus_sumb <- merge_otus_sum(ps_list, n = 0, taxal = "Species", binary = TRUE,
 con_cv, bin_cv, rarefy_size = 20000)
...
library(ggprism)
set.seed(123)
plot_sub_ordi2 <- function(ps, method_c, color_c, title_cs, min_depth = NULL, hellinger = TRUE) { #nolint
...
get_multi_betas <- function(ps_list, title_csv, method_c, color_c, min_depth=NULL) { #nolint
  p_ordi_list <- lapply(
    seq_along(ps_list), 
    function(i) {
...
plot_taxa_flow <- function(ps_melt, taxon_l, rel = FALSE, merge_unclassified = TRUE) { #nolint
  ...
  p_taxa <- ggplot(
    ...
    labs(x = "", y = ytitle) +
    facet_wrap(Group ~ Sample, scales = "free", nrow = 2) +
...
#p_upset
tag_merge_emu <- tag_merge[tag_merge$`Species: emu`==1 | tag_merge$`Genus: emu`==1,] #nolint
dir.create("table/emu", showWarnings = FALSE)
write.csv(tag_merge_emu, "table/emu/taxa_emu.csv", quote=FALSE, row.names=FALSE) #nolint
...
get_taxa_abun <- function(ps, taxon_l, taxa, rel = TRUE) {
  ...
  table_wide <- ps_melt %>%
    ...
    # row mean of non-zero values
    mutate_all(~replace(., . == 0, NA)) %>%
    mutate(mean= rowMeans(select_if(., is.numeric), na.rm = TRUE)) %>%
    #mutate(mean = rowMeans(.[, -1])) %>%
    arrange(desc(mean))
  return(table_wide)
}
emu_abun_s <- get_taxa_abun(ps_emu, taxon_l = "Species", taxa = tag_merge_emu$taxonomy[tag_merge_emu$`Species: emu`==1])#nolint
emu_abun_g <- get_taxa_abun(ps_emu, taxon_l = "Genus", taxa = tag_merge_emu$taxonomy[tag_merge_emu$`Genus: emu`==1])#nolint
emu_abun <- rbind.data.frame(emu_abun_s, emu_abun_g)
write.csv(emu_abun, "table/emu/taxa_emu_abun.csv", quote=FALSE, row.names=FALSE) #nolint
# min BLAST identity > 97
tag_merge_emu_no <- tag_merge[tag_merge$`Species: emu`==0 & tag_merge$`Genus: emu`==0,] #nolint
emu_abun_s_no_kmerCon <- get_taxa_abun(ps_kmerCon, taxon_l = "Species", taxa = tag_merge_emu_no$taxonomy[tag_merge_emu_no$`Species: kmerCon`==1])#nolint
emu_abun_s_no_miniCon <- get_taxa_abun(ps_miniCon, taxon_l = "Species", taxa = tag_merge_emu_no$taxonomy[tag_merge_emu_no$`Species: miniCon`==1])#nolint
emu_abun_s_no_isoCon <- get_taxa_abun(ps_isoCon, taxon_l = "Species", taxa = tag_merge_emu_no$taxonomy[tag_merge_emu_no$`Species: isoCon`==1])#nolint
emu_abun_g_no_kmerCon <- get_taxa_abun(ps_kmerCon, taxon_l = "Genus", taxa = tag_merge_emu_no$taxonomy[tag_merge_emu_no$`Genus: kmerCon`==1])#nolint
emu_abun_g_no_miniCon <- get_taxa_abun(ps_miniCon, taxon_l = "Genus", taxa = tag_merge_emu_no$taxonomy[tag_merge_emu_no$`Genus: miniCon`==1])#nolint
emu_abun_g_no_isoCon <- get_taxa_abun(ps_isoCon, taxon_l = "Genus", taxa = tag_merge_emu_no$taxonomy[tag_merge_emu_no$`Genus: isoCon`==1])#nolint
emu_abun_no_kmerCon <- rbind.data.frame(emu_abun_s_no_kmerCon, emu_abun_g_no_kmerCon) #nolint
emu_abun_no_miniCon <- rbind.data.frame(emu_abun_s_no_miniCon, emu_abun_g_no_miniCon) #nolint
emu_abun_no_isoCon <- rbind.data.frame(emu_abun_s_no_isoCon, emu_abun_g_no_isoCon) #nolint
# add mean abundance for taxa_emu_no
idx <- match(tag_merge_emu_no$taxonomy, emu_abun_no_kmerCon$taxonomy)
tag_merge_emu_no$mean_kmerCon <- emu_abun_no_kmerCon$mean[idx]
idx <- match(tag_merge_emu_no$taxonomy, emu_abun_no_miniCon$taxonomy)
tag_merge_emu_no$mean_miniCon <- emu_abun_no_miniCon$mean[idx]
idx <- match(tag_merge_emu_no$taxonomy, emu_abun_no_isoCon$taxonomy)
tag_merge_emu_no$mean_isoCon <- emu_abun_no_isoCon$mean[idx]
# taxa_level by number of `;`: species 6, genus 5
tag_merge_emu_no$taxonomy_level <- ifelse(str_count(tag_merge_emu_no$taxonomy, ";") == 5, "Genus", "Species") #nolint
write.csv(tag_merge_emu_no, "table/emu/taxa_emu_no_abun.csv", quote=FALSE, row.names=FALSE) #nolint
```
...
write.table(cors_wide, file = "table/emu/cor.csv", sep = ",", quote = FALSE, row.names = FALSE) #nolint
...
df_hitax <- df %>%
  filter(sseqid != "*") %>%
  mutate(qcov = (qend - qstart + 1) / length * 100) %>%
  #pident > 97, qcov > 99
  filter(pident >= 97 & qcov >= 99) %>%
...
# load all emu features
emu <- fread("table/emu/taxa_emu.csv", sep = ",") #nolint
# Prevotalla 7 to Prevotalla
emu$taxonomy_hit <- gsub("Prevotella\\s7\\s", "Prevotella ", emu$taxonomy, perl=TRUE) #nolint
emu$taxonomy_hit <- gsub("Burkholderia-Caballeronia-Paraburkholderia\\s", "Paraburkholderia ", emu$taxonomy_hit, perl=TRUE) #nolint
# RC9


emu_last <- gsub("^.*;", "", emu$taxonomy_hit)
emu$hitin_kmerCon <- sapply(emu_last, function(x) any(grepl(x, unique(df_hitax$Taxon[df_hitax$consensus == "kmerCon"])))) #nolint
emu$hitin_miniCon <- sapply(emu_last, function(x) any(grepl(x, unique(df_hitax$Taxon[df_hitax$consensus == "miniCon"])))) #nolint
emu$hitin_isoCon <- sapply(emu_last, function(x) any(grepl(x, unique(df_hitax$Taxon[df_hitax$consensus == "isoCon"])))) #nolint

#in hits species 
#id97: FALSE 16, TRUE 61; id99: FALSE 25, TRUE 52
table(emu$hitin_kmerCon[emu$`Species: emu` == 1])
#id:97: FALSE 16, TRUE 61; id99: FALSE 24, TRUE 53
table(emu$hitin_miniCon[emu$`Species: emu` == 1])
#id97: FALSE 18, TRUE 59; id99: FALSE 25, TRUE 52
table(emu$hitin_isoCon[emu$`Species: emu` == 1])
#in hits genus 
#id97: FALSE 11, TRUE 47; id99: FALSE 15, TRUE 43
table(emu$hitin_kmerCon[emu$`Genus: emu` == 1])
#id97: FALSE 12, TRUE 46; id99: FALSE 15, TRUE 43
table(emu$hitin_miniCon[emu$`Genus: emu` == 1])
#id97: FALSE 16, TRUE 42; id99: FALSE 19, TRUE 39
table(emu$hitin_isoCon[emu$`Genus: emu` == 1])

# get the relative abundance in emu profile
emu_abund <- read.csv("table/emu/taxa_emu_abun.csv", header = TRUE, stringsAsFactors = FALSE) #nolint
# add if covered in blast hit 
idx <- match(emu_abund$taxonomy, emu$taxonomy)
emu_abund$hit_cover_kmerCon <- emu$hitin_kmerCon[idx]
emu_abund$hit_cover_miniCon <- emu$hitin_miniCon[idx]
emu_abund$hit_cover_isoCon <- emu$hitin_isoCon[idx]
emu$tax_level <- ifelse(emu$`Species: emu` == 1, "Species", "Genus")
emu_abund$taxonomy_level <- emu$tax_level[idx]
# use cleand taxonomy
emu_abund$taxonomy <- emu$taxonomy_hit[idx]
write.csv(emu_abund, "table/emu/taxa_emu_abun_hit.csv", quote = FALSE, row.names = FALSE) #nolint

# >0.1%
# species: FALSE 1, TRUE 31; Criibacterium bergeronii
# not in filtered QIIME SILVA database
# NCBI BLAST, cover,pid,accession:
# kmerCon: OTU_41, 99%,98.69%,NR_164617.1
# miniCon: OTU_249, 98%,98.49%,NR_164617.1
# isoCon:
# OTU_195, 98%,98.42%,NR_164617.1
# OTU_791, 98%,98.55%,NR_164617.1
# OTU_866: 99%,98.48%,NR_164617.1
# genus:  FALSE 2, TRUE 27
# Criibacterium, see above
# Rikenellaceae RC9 gut group, rejected by identity ~90%
# kmerCon: OTU_17
# miniCon: OTU_303
# isoCon: OTU92, OTU_266
# kmerCon
table(emu_abund$hit_cover_kmerCon[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Species"]) #nolint
emu_abund$taxonomy[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Species" & emu_abund$hit_cover_kmerCon == FALSE] #nolint
# genus # FALSE 1 (Criibacterium)
table(emu_abund$hit_cover_kmerCon[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Genus"]) #nolint
emu_abund$taxonomy[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Genus" & emu_abund$hit_cover_kmerCon == FALSE] #nolint
# miniCon
table(emu_abund$hit_cover_miniCon[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Species"]) #nolint
emu_abund$taxonomy[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Species" & emu_abund$hit_cover_miniCon == FALSE] #nolint
table(emu_abund$hit_cover_miniCon[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Genus"]) #nolint
emu_abund$taxonomy[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Genus" & emu_abund$hit_cover_miniCon == FALSE] #nolint
# isoCon
table(emu_abund$hit_cover_isoCon[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Species"]) #nolint
emu_abund$taxonomy[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Species" & emu_abund$hit_cover_isoCon == FALSE] #nolint
table(emu_abund$hit_cover_isoCon[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Genus"]) #nolint
emu_abund$taxonomy[emu_abund$mean > 0.1 & emu_abund$taxonomy_level == "Genus" & emu_abund$hit_cover_isoCon == FALSE] #nolint
```

```{r, cor-mapping-clustering, include=FALSE, message=FALSE}
ps_kmerCon2 <- ps_from_laca( # nolint
...
ps_miniCon2 <- ps_from_laca( # nolint
...
ps_isoCon2 <- ps_from_laca( # nolint
...
# reuse ps_melt_abundance, dt_cor
ps_otu_abundance <- function(ps, rel = TRUE) {
  if (rel == TRUE) {
    ps <- transform_sample_counts(ps, function(x) x / sum(x) * 100)
  }
  otutable <- data.frame(otu_table(ps))
  otutable$taxonomy <- rownames(otutable)
  #make taxonomy as the first column
  otutable <- otutable[, c(ncol(otutable), 1:(ncol(otutable)-1))]
  return(otutable)
}
#get_otu_abundance(ps_kmerCon)
#ps_melt_abundance(ps_kmerCon, taxon_l = "Species", rel = TRUE)
ps_cor_ext <- function(ps1, ps2, tag, taxon_l = "Species", rel = TRUE, nzeros_pct = 0.3) { #nolint
  if (taxon_l == "OTU"){
    widetab1 <- ps_otu_abundance(ps1, rel = rel)
    widetab2 <- ps_otu_abundance(ps2, rel = rel) 
  } else {
    widetab1 <- ps_melt_abundance(ps1, taxon_l = taxon_l, rel = rel)
    widetab2 <- ps_melt_abundance(ps2, taxon_l = taxon_l, rel = rel)
  }
...
ps_cor_ext2 <- function(ps1, ps2, tag, taxon_l = c("OTU", "Genus", "Species"), rel = TRUE, nzeros_pct = 0.3) { #nolint
  cors <- data.frame()
  for (taxon in taxon_l) {
    cors_taxon <- ps_cor_ext(ps1, ps2, tag, taxon_l = taxon, rel = rel, nzeros_pct = nzeros_pct) #nolint
    cors <- rbind(cors, cors_taxon)
  }
  return(cors)
}

# relative to the minimap2 mapping profile
ps_list <- list(
  ps_kmerCon,
  ps_miniCon,
  ps_isoCon
...
for (i in 1:length(ps_list)) {
  ...
  ps2 <- ps_list2[[i]]
  tag <- methods[i]
  cors2_ps <- ps_cor_ext2(ps, ps2, tag = tag)
  cors2 <- rbind(cors2, cors2_ps)
}
# split cor and p by method into 4 columns, and keep other columns
cors2_pearson <- cors2 %>% filter(method == "pearson") %>% select(-method) %>% dplyr::rename(cor_pearson = cor, p_pearson = p) #nolint
cors2_spearman <- cors2 %>% filter(method == "spearman") %>% select(-method) %>% dplyr::rename(cor_spearman = cor, p_spearman = p) #nolint
cors2_wide <- cors2_pearson %>% left_join(cors2_spearman, by = c("taxonomy", "taxon_l", "tag", "mean")) #nolint
write.table(cors2_wide, file = "table/emu/cor_mapping_clustering.csv", sep = ",", quote = FALSE, row.names = FALSE) #nolint
# pearson|spearman cor scatter plot
# color: profile method
...
p_cor2 <- cors2_wide %>% 
  ...
  facet_grid(
    factor(taxon_l, levels = c("Genus", "Species", "OTU")) 
    ~ tag, scale="free", space="free") +
  theme_prism() + 
  theme(
    legend.title = element_text(),
...
mapply(ggsave2, pnames_l, ps_l, h = 12, w = 16)
ggsave("figure/emu/p2_betas.png", p_multi_betas, height = 10, width = 10) #nolint
...
ggsave("figure/emu/p_cor_mapping_clustering.png", p_cor2, height = 8, width = 12) #nolint
...
ggsave("figure/emu/p_upset_cor.png", p_upset_cor, height = 10, width = 12) #nolint
```
