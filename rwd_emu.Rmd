...
ps_kmerCon <- ps_from_laca( # nolint
  ...
  hits_tsv = "data/emu/kmerCon_out/taxonomy/q2blas/hits.tsv",
  min_id = 97
  )
ps_miniCon <- ps_from_laca( # nolint
  ...
  hits_tsv = "data/emu/miniCon_out/taxonomy/q2blas/hits.tsv",
  min_id = 97
  )
ps_isoCon <- ps_from_laca( # nolint
  ...
  hits_tsv = "data/emu/isoCon_out/taxonomy/q2blas/hits.tsv",
  min_id = 97
  )

# load emu profiles manually
countab <- read.table("data/emu/emu_fastq_b0.tsv", header = T, sep = "\t", row.names = 1, comment.char = "") # nolint
...
p_multi_betas <- patchwork::wrap_plots(c(multi_wunifrac, multi_bray), guides = "collect", nrow = 4) #nolint
...
ps_kmerCon0 <- ps_from_laca( # nolint
  count_tsv = "data/emu/kmerCon_out/count_matrix.tsv",
  meta_tsv = "data/emu/mapping_nanopore10000.tsv",
  tax_tsv = "data/emu/kmerCon_out/taxonomy.tsv",
  tree_nwk = "data/emu/kmerCon_out/tree.nwk",
  rep_fasta = "data/emu/kmerCon_out/rep_seqs.fasta",
...
dir.create("table/emu", showWarnings = FALSE)
write.csv(tag_merge_emuonly, "table/emu/taxa_emuonly01.csv", quote=FALSE, row.names=FALSE) #nolint
...
# no emu only genus with relative abundance > 0.1%
...
emu_abun <- emu_abun_s
write.csv(emu_abun, "table/emu/taxa_emuonly_abun01.csv", quote=FALSE, row.names=FALSE) #nolint
```
...
cors_pearson <- cors %>% filter(method == "pearson") %>% select(-method) %>% rename(cor_pearson = cor, p_pearson = p) #nolint
cors_spearman <- cors %>% filter(method == "spearman") %>% select(-method) %>% rename(cor_spearman = cor, p_spearman = p) #nolint
...
#in hits species #FALSE 17, TRUE 49
#in hits genus #FALSE 12, TRUE 6
...
# >0.1%
emuonly01 <- fread("table/emu/taxa_emuonly01.csv", sep = ",") #nolint
emuonly01_last <- gsub("^.*;", "", emuonly01$taxonomy)
#table(sapply(emuonly01_last, function(x) any(grepl(x, unique(df_hitax$Taxon))))) #all covered in blast hit
```
...
png("figure/emu/p_upset01.png", height = 6, width = 12, units = "in", res = 300) #nolint
p_upset
dev.off()
