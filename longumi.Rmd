...
dt_cutoff <- dt_qc_f %>%
  ...
  mutate(cutoff=case_when(
    consensus %in% c("kmerCon", "miniCon", "isoCon") ~ 15, #nobin
    TRUE ~ 4
  ), y= Inf) %>%
  mutate(x = case_when(
...
df$cutoff <- case_when(
  ...
  df$consensus %in% c("kmerCon", "miniCon", "isoCon") ~ 15, # chimera
  TRUE ~ 3
)
...
ggsave("./figure/longumi/longumi.png", p_longumi, width = 12, height = 8) #nolint
ggsave("./figure/longumi/longumi_f.png", p_longumi_f, width = 12, height = 8) #nolint
