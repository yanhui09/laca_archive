...
dir.create("data/longumi", showWarnings = FALSE)
qc_complie <- function(
  qc_dir,
  consensus_prefix,
...
fs <- list.files(
  path = "/mnt/md0/UMI16S/cp2/latest/longumi/qc/sub100000",
...
df$data <- gsub("^.*qc/sub100000/(.*)/(.*)/qc$", "\\1", df$qc_dir) #nolint
df$consensus <- gsub("^.*qc/sub100000/(.*)/(.*)/qc$", "\\2", df$qc_dir) #nolint
df$consensus_prefix <- case_when(
  df$consensus == "umiCon" ~ "umiCon_trimmed",
  TRUE ~ df$consensus
)
df$read_data <- case_when(
  df$data == "pb_ccs" ~ "ccs_sub100000_s11.fa",
  df$data == "zymock_r9" ~ "ERR3336963_1sub100000_s123.fa",
  df$data == "zymock_r10" ~ "ERR3813594_1sub100000_s123.fa"
)
...
dt_cutoff <- dt_qc_f %>%
  ...
  mutate(cutoff=case_when(
    consensus %in% c("kmerCon", "miniCon") ~ 5, #nolint
    consensus == "isoCon" ~ 15, # nolint
    TRUE ~ 3
  ), y= Inf) %>%
  mutate(x = case_when(
...
dir.create("./figure/longumi", showWarnings = FALSE, recursive = TRUE)
ggsave("./figure/longumi/size_err.png", p_size_err, width = 12, height = 8) #nolint
ggsave("./figure/longumi/size_err_f.png", p_size_err_f, width = 12, height = 8) #nolint
```
...
df$consensus <- gsub("^.*longumi/(.*)/(.*)_qc.Rdata", "\\2", df$qc)
df$cutoff <- case_when(
  df$consensus %in% c("kmerCon", "miniCon") ~ 5, #nolint
  df$consensus == "isoCon" ~ 15, # nolint
  TRUE ~ 3
)
...
if (!file.exists("./data/longumi_sumerr.Rdata")) {
  save(dt_sum, dt_sum_f, file = "./data/longumi_sumerr.Rdata")
}
```
...
write.csv(dt_sum[, -1], "./table/longumi/umi_sumerr.csv", quote = FALSE, row.names = FALSE) #nolint
write.csv(dt_sum_f[, -1], "./table/longumi/umi_sumerr_f.csv", quote = FALSE, row.names = FALSE) #nolint
...
```{r, ggsave}
dir.create("./figure/longumi", showWarnings = FALSE)
ggsave("./figure/longumi/stat_overview.png", p_stat_overview, width = 12, height = 4) #nolint
ggsave("./figure/longumi/stat_overview_f.png", p_stat_overview_f, width = 12, height = 4) #nolint
ggsave("./figure/longumi/stat_consensus.png", p_stat_consensus, width = 12, height = 4) #nolint
ggsave("./figure/longumi/stat_consensus_f.png", p_stat_consensus_f, width = 12, height = 4) #nolint
ggsave("./figure/longumi/stat_err.png", p_stat_err, width = 12, height = 4) #nolint
ggsave("./figure/longumi/stat_err_f.png", p_stat_err_f, width = 12, height = 4) #nolint
# chimera all
ggsave("./figure/longumi/chimerate_all.png", p_chimerate_all, width = 8, height = 6) #nolint
# all 3
...
ggsave("./figure/longumi/longumi3.png", p_longumi3, width = 12, height = 12) #nolint
ggsave("./figure/longumi/longumi3_f.png", p_longumi3_f, width = 12, height = 12) #nolint
```