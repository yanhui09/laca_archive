...
dt_cutoff <- dt_qc_f %>%
  ...
  mutate(cutoff=case_when(
    ...
    consensus == "kmerCon" ~ 80,
    consensus == "miniCon" ~ 60,
    consensus == "isoCon" ~ 20,
    TRUE ~ 4
  ), y= Inf) %>%
  mutate(x = case_when(
    consensus %in% c("kmerCon", "miniCon") ~ cutoff + 20,
...
df$cutoff <- case_when(
  df$data == "zymock_r10" & df$consensus == "umiCon" ~ 15,
  df$data == "zymock_r9" & df$consensus == "umiCon" ~ 25,
  df$consensus == "kmerCon" ~ 80,
  df$consensus == "miniCon" ~ 60,
  df$consensus == "isoCon" ~ 20,
  TRUE ~ 4
)
...
#rds <- "./data/longumi1000000/pb_ccs/umiCon_qc.Rdata"
#load(rds)
#table(qc$chimera == "Y")
#breaks <- c(seq(1, 60, 3), Inf)
#lu_artefact_plot(qc, breaks = breaks)
...
stat_bar <- function(df, column, y_lab, y_int=NULL) {
  ...
  {if(! is.null(y_int))geom_hline(yintercept = y_int, linetype = "dashed")} + #nolint
