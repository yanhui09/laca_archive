...
# only take .*/(pb_ccs|zymock_r9|zymock_r10)/(umiCon|isoCon|kmerCon|miniCon)/qc/bin_map.txt
fs <- grep(pattern = ".*/(pb_ccs|zymock_r9|zymock_r10)/(umiCon|isoCon|kmerCon|miniCon)/qc/bin_map.txt", x = fs, value = TRUE) #nolint
...
dt_cutoff$ylabel_q20[dt_cutoff$consensus == "kmerCon"] <- 2
dt_cutoff$label_q20[dt_cutoff$consensus == "kmerCon"] <- "Q20"
dt_cutoff$ylabel_q30[dt_cutoff$consensus == "kmerCon"] <- 0.5
...
ggsave("./figure/longumi1000000/size_err.png", p_size_err, width = 14, height = 10) #nolint
ggsave("./figure/longumi1000000/size_err_f.png", p_size_err_f, width = 14, height = 10) #nolint
```
...
df$consensus <- gsub("^.*longumi1000000/(.*)/(.*)_qc.Rdata", "\\2", df$qc)
df$cutoff <- case_when(
  # 250x, 200x for kmerCon, miniCon to get more refs
...
stat_bar_scale <- function(df, column, y_lab, y_int = NULL) {
  ...
  scale_y_continuous(
    ...
    breaks = c(1, 10, 100, 1000, 5000, 10000, 30000),
    )
  return(p)
}
...
# isoCon/umiCon ceiling ONTR10.3: 0.75/0.15=5
# isoCon/umiCon ceiling ONTR9.4.1: 0.9/0.3=3
...
stat_col <- function(dt, column, y_lab, title, y_int=NULL) {
  ...
  scale_y_continuous(
    trans = scales::sqrt_trans(),
    breaks = c(0, 0.01, 0.1, 0.3, 0.5, 1, 2, 3),
    ) +
...
ggsave("./figure/longumi1000000/stat_err.png", p_stat_err, width = 12, height = 6) #nolint
ggsave("./figure/longumi1000000/stat_err_f.png", p_stat_err_f, width = 12, height = 6) #nolint
# chimera all
...
ggsave("./figure/longumi1000000/longumi3.png", p_longumi3, width = 12, height = 15) #nolint
ggsave("./figure/longumi1000000/longumi3_f.png", p_longumi3_f, width = 12, height = 15) #nolint
```