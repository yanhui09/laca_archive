...
filter_taxa <- function(tax_tsv, hits_tsv, min_id) {
...
ps_from_laca <- function(count_tsv, meta_tsv, tax_tsv, tree_nwk, rep_fasta, hits_tsv, min_id, spe_=TRUE, rarefy_size=NULL) { # nolint
  ...
  tax <- filter_taxa(tax_tsv, hits_tsv, min_id)
  rownames(tax) <- tax$V1
...
```{r, taxa-bar, include=FALSE, message=FALSE}
...
taxa_plot <- function(ps, taxon_l) {
  ps_rel <- transform_sample_counts(ps, function(x) x / sum(x) * 100)
  # agglomerate taxa
  glom <- tax_glom(ps_rel, taxrank = taxon_l, NArm = FALSE)
  ps_melt <- psmelt(glom)
  # change to character for easy-adjusted level
  taxon <- eval(substitute(get(taxon_l)), ps_melt)
  ps_melt$taxon_l <- as.character(taxon)
  ps_melt <- ps_melt %>%
    group_by(Group, taxon_l) %>%
    mutate(median = mean(Abundance))
  # select grouped median > 1
  keep <- unique(ps_melt$taxon_l[ps_melt$median > 0.1])
  ps_melt$taxon_l[!(ps_melt$taxon_l %in% keep)] <- "< 0.1%"
  # to get the same rows together
  ps_melt_sum <- ps_melt %>%
    group_by(selfID, Zymo_series, Spike_in, taxon_l) %>%
  ...
  p_taxa <- ggplot(
    ps_melt_sum,
     aes(x = selfID, y = Abundance, fill = taxon_l)) +
    geom_bar(stat = "summary", aes(fill = taxon_l), fun = "mean") +
    labs(x = "", y = "%") +
    facet_wrap(. ~ Spike_in, scales = "free_x", nrow = 1) +
...
p_otus_count <- df_otus_count %>%
  ...
  geom_segment(data = data.frame(xs=0,xe=50,ys=97,ye=97,subsample="Subsample10000"), aes(x=xs, xend=xe, y=ys, yend=ye), linetype = "dashed") + #nolint
  geom_segment(data = data.frame(xs=50,xe=50,ys=-Inf,ye=97,subsample="Subsample10000"), aes(x=xs, xend=xe, y=ys, yend=ye), linetype = "dashed") + #nolint
  geom_segment(data = data.frame(xs=0,xe=500,ys=97,ye=97,subsample="Subsample100000"), aes(x=xs, xend=xe, y=ys, yend=ye), linetype = "dashed") + #nolint
  geom_segment(data = data.frame(xs=500,xe=500,ys=-Inf,ye=97,subsample="Subsample100000"), aes(x=xs, xend=xe, y=ys, yend=ye), linetype = "dashed") + #nolint
...
library(patchwork)
p_merge <- p_otus_sum + p_otus_sumb + p_taxa_flow2 + p_blast + plot_layout(nrow = 2) + plot_annotation(tag_levels = "a")  #nolint
