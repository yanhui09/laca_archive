...
df2 <- df %>%
  ...
  reframe(
    ...
    pident = max(pident),
    # take min cov per OTU
    cov = min(gapopen / length * 100),
...
p_blast <- df2 %>%
  ...
  geom_quasirandom(
    aes(
      color = cov,
      shape = fct_relevel(
        LCA, "Species", "Genus", "Order", "Class" #no Family
      )
    )) +
...
pct_otu <- df2 %>%
  ...
  summarise(
    n = n(),
    unclassified = sum(tax == "Unassigned") / n * 100,
    `min identity > 97%` = sum(min_pident >= 97) / n * 100,
    `min identity > 99%` = sum(min_pident >= 99) / n * 100,
  )
pct_otu
...
ps <- list(p_otus_sum, p_otus_sumb, p_taxa_flow, p_taxa_flow2, p_taxa_flow2g, p_blast) #nolint
pnames <- c("p1_otus", "p1_otusb", "p3_bars_ab", "p4_bars", "p5_barg", "p6_blast") #nolint
...
library(patchwork)
p_merge <- p_otus_sum + p_otus_sumb + p_taxa_flow2 + p_blast + plot_layout(ncol = 2) + plot_annotation(tag_levels = "a")  #nolint
ggsave("figure/zymock/p_merge.png", p_merge, height = 12, width = 18) #nolint
```
